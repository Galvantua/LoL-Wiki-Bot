...
module.exports = {
	...
	async execute(interaction) {
		...
		const request = await fetch(`https://cdn.merakianalytics.com/riot/lol/resources/latest/en-US/champions/${championName}.json`)
		.catch((err) => {
		...
		if (level != "n") {
			...
			for (let i = 0; i < growthStats.length; i++) {
				...
				flatStats[i] = flatStats[i] + growth * (level - 1) * (0.7025 + 0.0175 * (level - 1));

				flatStats[i] = Math.round((flatStats[i] + Number.EPSILON) * 100) / 100;
			...
			for (let i = 0; i < growthStats.length; i++) {
				flatStats[i] = flatStats[i].toString() + " + " + growthStats[i].toString();
			};
